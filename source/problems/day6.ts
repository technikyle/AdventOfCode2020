...
type Answers = { [answer: string]: number};
type GroupAnswers = { answer: Answers, total: number}

function parseLine(x: string): Answers {
    var result: Answers = {}
    for (let char of x) {
        result[char] = 1
    }
...
let parseGroup = (line: string) => {
    var result: Answers = {}
    let individualLines = line.split('\n').filter(x => x != '')
    individualLines.forEach((line) => {
        let answer = parseLine(line)
        for (let key in answer) {
            result[key] ??= 0
            result[key] += answer[key]
        }
    })
    return {answer: result, total: individualLines.length}
}

let groups: GroupAnswers[] = input.split('\n\n').filter(x => x != '').map((line) => {
    let group = parseGroup(line)
    return group
})

export function p1(): number {
    function calcAnswerTrues(answer: GroupAnswers): number {
        var answerCount = 0
        for (let key in answer.answer) {
            answerCount += answer.answer[key] != null ? 1 : 0
        }
        return answerCount
    }
    let result = groups.map(calcAnswerTrues).reduce((a, b) => a + b)
    return result
}
...
export function p2(): number {
    function calcAnswerTrues(answer: GroupAnswers): number {
        var result = 0
        for (let key in answer.answer) {
            result += (answer.answer[key] == answer.total ? 1 : 0)
        }
        return result
    }
    let result = groups.map(calcAnswerTrues).reduce((a, b) => a + b)
