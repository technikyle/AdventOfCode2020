...
function parseLine(x: string): Passport {
    let arrays = x.split(' ').map(x=>x.split('\n'))
    const flattenedArray = ([] as string[]).concat(...arrays);

    var object: { [key: string]: string; } = {};

    flattenedArray.filter(x => x != '').forEach(x => { 
        let [key, value] = x.split(":")
        object[key] = value
    })

    let result = object as Passport
    return result
}

let lines: Passport[] = input.split('\n\n').filter(x => x != '').map((line) => {
...
export function p1(): number {
    ...
    let result = lines.filter(passes).length
...
export function p2(): number {
    function validateByr(x?: string): boolean {
        if (x == null) { return false }
        let result = x.length == 4 && +x >= 1920 && +x <= 2002
        return result
    }

    function validateIyr(x?: string): boolean {
        if (x == null) { return false }
        let result = x.length == 4 && +x >= 2010 && +x <= 2020
        return result
    }

    function validateEyr(x?: string): boolean {
        if (x == null) { return false }
        let result = x.length == 4 && +x >= 2020 && +x <= 2030
        return result
    }

    function validateHgt(x?: string): boolean {
        if (x == null) { return false }
        let value = x.slice(0, x.length - 2)
        let distance = x.slice(-2)
        var result = false
        if (distance == 'cm') {
    ...
    function validateHcl(x?: string): boolean {
        if (x == null) { return false }
        let patt1 = /^[a-f0-9]*$/g;
        let test = (x.slice(1).match(patt1) || [])
        let result = x[0] == "#" && test.length == 1 && test[0].length == 6;
        return result
    }

    function validateEcl(x?: string): boolean {
        if (x == null) { return false }
        let colors = ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']
        let result = colors.includes(x)
        return result
    }

    function validatePid(x?: string): boolean {
        if (x == null) { return false }
        let patt1 = /^[0-9]*$/g;
        let test = (x.match(patt1) || [])
        let result = test.length == 1 && test[0].length == 9;
        return result
    }

    function validateCid(x?: string): boolean {
        return true
    }
    ...
    let passes = (passport: Passport) => { 
        ...
        validateCid(passport.cid)
    }

    return lines.filter(passes).length
    // return result as number
}